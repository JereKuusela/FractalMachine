  ...
  public partial class FractalForm : Form
  {
    ...
    private void element_MouseHover(object sender, EventArgs e)
    {
      ...
      if (name == labelCenter.Name || name == numericCenterX.Name || name == numericCenterY.Name)
        ((MDIForm)MdiParent).setHelpText("Asettaa kuvan keskipisteen. Arvojen kasvattaminen siirtää kuvaa vasemmalle/alas.");
      else if (name == labelZoom.Name || name == numericZoomX.Name || name == numericZoomY.Name)
        ((MDIForm)MdiParent).setHelpText("Asettaa kuvan zoomauksen. Arvojen kasvattaminen venyttää kuvaa.");
      else if (name == labelSize.Name || name == numericSizeX.Name || name == numericSizeY.Name)
        ((MDIForm)MdiParent).setHelpText("Asettaa piirtoalueen koon (pikselien määrä).");
      else if (name == labelIteration.Name || name == numericIteration.Name)
        ((MDIForm)MdiParent).setHelpText("Asettaa algoritmin kierrosten lukumäärän. Arvon kasvattaminen lisää kuvan monimutkaisuutta.");
        ...
        ((MDIForm)MdiParent).setHelpText("Asettaa Newton-fraktaalin algoritmin siirtymän. Vaikuttaa kuvan muotoon.");
      else if (name == buttonPalett.Name)
        ((MDIForm)MdiParent).setHelpText("Avaa ikkunan, josta voit valita kuvassa käytettävät värit ja niiden määrän.");
      else if (name == buttonRender.Name)
        ((MDIForm)MdiParent).setHelpText("Piirtää tai keskeyttää kuvan piirron.");
        ...
        ((MDIForm)MdiParent).setHelpText("Tallentaa kuvan asetusarvot, jotta sen voi avata tällä ohjelmalla myöhemmin.");
      else if (name == checkBoxInterpolation.Name)
        ((MDIForm)MdiParent).setHelpText("Määrittää yhdistetäänkö viereisiä värejä tasaisemman lopputuloksen saamiseksi.");
      else if (name == checkBoxPreview.Name)
        ((MDIForm)MdiParent).setHelpText("Määrittää piirretäänkö kuva automaattisesti asetusten muuttamisen jälkeen.");
      else if (name == checkBoxFilter.Name)
        ((MDIForm)MdiParent).setHelpText("Määrittää pehmennetäänkö jyrkkiä rajoja. Sumentaa kuvaa.");
      else if (name == pictureBox1.Name)
      {
        if (pictureHelpCounter == 0)
          ((MDIForm)MdiParent).setHelpText("Hiiren vasen nappi siirtää valitun pisten kuvan keskelle.");
        else if (pictureHelpCounter == 1)
          ((MDIForm)MdiParent).setHelpText("Hiiren oikean napin pitäminen pohjassa zoomaa kuvan valitulle alueelle.");
        else
          ((MDIForm)MdiParent).setHelpText("Hiiren keskinappi asettaa lisäasetuksen Julia- tai Newton-fraktaalille.");
