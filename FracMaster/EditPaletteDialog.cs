...
namespace FracMaster
{
  public partial class EditPaletteDialog : Form
  {
    ...
    private void ApplyPaletteToForm()
    {
      if (InvokeRequired)
      {
        Invoke(new MethodInvoker(ApplyPaletteToForm));
      }
      else
      {
        ...
        if (palette != null)
        {
          ...
          panel1.Width = palettewidth;
          panel1.Height = palettewidth;
    ...
    void UpdatePaletteBox()
    {
      if (InvokeRequired)
      {
        Invoke(new MethodInvoker(UpdatePaletteBox));
      }
      else
      {
    ...
    private void numericColorCount_ValueChanged(object sender, EventArgs e)
    {
